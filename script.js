document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderTodayHistory() {
        ...
        const entriesToShow = todayEntries.slice(-3).reverse();
        ...
        if (todayEntries.length > 3) {
            ...
            viewMoreItem.textContent = `+${todayEntries.length - 3} more entries`;
    ...
    function updatePercentageCircle() {
        ...
        percentageCircleEl.style.strokeDasharray = `${circumference * percentage} ${circumference * (1 - percentage)}`;
    }
        ...
        if (period === 'today') {
            ...
            state.history.forEach(entry => {
                ...
                if (entryDate >= oneWeekAgo) {
                    const dayIndex = Math.floor((entryDate - oneWeekAgo) / (1000 * 60 * 60 * 24));
    ...
    function renderDetailedHistory(period) {
        ...
        for (const [dateStr, dayEntries] of Object.entries(entriesByDay)) {
            ...
            dayHeader.innerHTML = `
                ...
                <span class="total">Total: ${dayTotal}ml</span>
            `;
    ...
    function setupReminders() {
        ...
        if (state.settings.reminderFrequency === 0) {
            return;
        }
        ...
        window.reminderInterval = setInterval(() => {
            ...
            // Show notification only if within wake hours
